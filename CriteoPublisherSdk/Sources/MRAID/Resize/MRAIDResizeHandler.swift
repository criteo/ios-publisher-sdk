...
struct MRAIDResizeHandler {
    ...
    func resize(delegate: MRAIDResizeHandlerDelegate) throws {
        ...
        func updateResizeMessageToFit(containerSize: CGSize, positionInContainer: CGPoint, with message: MRAIDResizeMessage) throws -> MRAIDResizeMessage {
            ...
            if positionY + resizeMessage.height > containerHeight {
        ...
        func verifyCloseAreaOutOfBounds(containerSize: CGSize, positionInContainer: CGPoint, message: MRAIDResizeMessage, autoRotate: Bool) throws -> MRAIDResizeMessage {
            let adjustedPosition = CGPoint(x: positionInContainer.x + CGFloat(message.offsetX), y: positionInContainer.y + CGFloat(message.offsetY))
        ...
        func verifyMinSize(message: MRAIDResizeMessage) throws {
            guard
                resizeMessage.height >= Int(Constants.minWidth),
                resizeMessage.width >= Int(Constants.minWidth) else {
                throw ResizeError.exceedingMinSize
            }
        }

        func closeAreaPositionInAdContainer(with newSize: CGSize, for customClosePosition: MRAIDCustomClosePosition) -> CGPoint {
