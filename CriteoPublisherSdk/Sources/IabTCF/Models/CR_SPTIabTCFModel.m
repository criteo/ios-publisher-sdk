...
- (BOOL)isPublisherCustomPurposeLegitInterestGivenFor:(int)purposeId {
  return [self booleanInBitString:self.publisherTCParsedCustomPurposesLegitmateInterest
                            forId:purposeId];
}
...
- (PublisherRestrictionType)publisherRestrictionTypeForVendor:(int)vendorId
  ...
  if (!parsedVendorsPubRest || parsedVendorsPubRest.length == 0 ||
      parsedVendorsPubRest.length < vendorId) {
    return Restriction_Undefined;
  }
  NSInteger restIntvalue =
      [[parsedVendorsPubRest substringWithRange:NSMakeRange(vendorId - 1, 1)] integerValue];
...
- (NSDictionary *)asJson {
  ...
  [result setValue:self.publisherTCParsedPurposesConsents
            forKey:@"publisherTCParsedPurposesConsents"];
  [result setValue:self.publisherTCParsedPurposesLegitmateInterest
            forKey:@"publisherTCParsedPurposesLegitmateInterest"];
  [result setValue:self.publisherTCParsedCustomPurposesConsents
            forKey:@"publisherTCParsedCustomPurposesConsents"];
  [result setValue:self.publisherTCParsedCustomPurposesLegitmateInterest
            forKey:@"publisherTCParsedCustomPurposesLegitmateInterest"];

  NSMutableArray *pubRestArray =
      [[NSMutableArray alloc] initWithCapacity:self.publisherRestrictions.count];
