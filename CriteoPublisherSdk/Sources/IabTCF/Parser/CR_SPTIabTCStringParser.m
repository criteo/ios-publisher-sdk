...
+ (void)completeModel:(CR_SPTIabTCFModel *)model withCoreString:(NSString *)coreString {
    ...
    NSInteger version = [CR_SPTIabTCFUtils BinaryToDecimal:binaryCharBuffer fromIndex:VERSION_BIT_OFFSET length:VERSION_BIT_LENGTH];
    ...
    model.cmpId = [CR_SPTIabTCFUtils BinaryToDecimal:binaryCharBuffer fromIndex:CMP_ID_BIT_OFFSET length:CMP_ID_BIT_LENGTH];
    model.cmpVersion = [CR_SPTIabTCFUtils BinaryToDecimal:binaryCharBuffer fromIndex:CMP_VERSION_BIT_OFFSET length:CMP_VERSION_BIT_LENGTH];
    model.consentScreen = [CR_SPTIabTCFUtils BinaryToDecimal:binaryCharBuffer fromIndex:CONSENT_SCREEN_BIT_OFFSET length:CONSENT_SCREEN_BIT_LENGTH];
    ...
    model.vendorListVersion = [CR_SPTIabTCFUtils BinaryToDecimal:binaryCharBuffer fromIndex:VENDOR_LIST_VERSION_BIT_OFFSET length:VENDOR_LIST_VERSION_BIT_LENGTH];
    ...
    if (version == 1) {
        ...
        model.policyVersion = [CR_SPTIabTCFUtils BinaryToDecimal:binaryCharBuffer fromIndex:POLICY_VERSION_BIT_OFFSET length:POLICY_VERSION_BIT_LENGTH];
        model.isServiceSpecific = [CR_SPTIabTCFUtils BinaryToBoolean:binaryCharBuffer atIndex:IS_SERVICE_SPECIFIC_BIT];
        model.useNonStandardStack = [CR_SPTIabTCFUtils BinaryToBoolean:binaryCharBuffer atIndex:USE_NON_STANDART_STACK_BIT];
        ...
        model.purposeOneTreatment = [CR_SPTIabTCFUtils BinaryToBoolean:binaryCharBuffer atIndex:PURPOSE_ONE_TREATMENT_BIT];
...
+ (void)completeModel:(CR_SPTIabTCFModel *)model forType:(SPTTcfDecoderVendorStringType)type withVendorsString:(NSString *)vendors {
    ...
    CR_SPTIabTCFSegmentType segmentType = [CR_SPTIabTCFUtils BinaryToDecimal:binaryCharBuffer fromIndex:0 length:SEGMENT_TYPE_BIT_LENGTH];
    SPTDecodedStringAndOffset *consentAndOffset;
...
+ (void)completeModel:(CR_SPTIabTCFModel *)model withPublisherPurposesTransparencyConsent:(NSString *)segmentString {
    ...
    NSInteger numCustomPurposes = [CR_SPTIabTCFUtils BinaryToDecimal:binaryCharBuffer fromIndex:PUBLISHER_NUM_CUSTOM_PURPOSES_BIT_OFFSET length:PUBLISHER_NUM_CUSTOM_PURPOSES_BIT_LENGTH];
...
+ (NSDate *)decodeDateFromBinary:(unsigned char*)binaryCharBuffer fromIndex:(int)startIndex length:(int)length {
    NSInteger deciSecondsEpoch = [CR_SPTIabTCFUtils BinaryToDecimal:binaryCharBuffer fromIndex:startIndex length:length];
...
+ (NSString *)decode6BitCharStringFromBinary:(unsigned char*)binaryCharBuffer fromIndex:(int)startIndex length:(int)length {
    ...
    if (length%charBitLenght != 0) {
        NSDebugLog(@"CR_SPTIabConsentStringParser: Invalid 6 bit char encoded string (not a 6 multiple)");
        return @"";
    }
    int characterNumber = length/charBitLenght;
    NSString *result = @"";
    for (int i = 0; i<characterNumber; i++) {
        NSInteger letter = a + [CR_SPTIabTCFUtils BinaryToDecimal:binaryCharBuffer fromIndex:startIndex + charBitLenght*i length:charBitLenght];
...
+ (SPTDecodedStringAndOffset *)decodeVendorBitChainStringFromBinary:(unsigned char*)binaryCharBuffer fromIndex:(int)startIndex version1:(BOOL)v1 {
    ...
    NSInteger maxVendorId = [CR_SPTIabTCFUtils BinaryToDecimal:binaryCharBuffer fromIndex:startIndex length:MAX_VENDOR_ID_BIT_LENGTH];
...
+ (SPTDecodedPublisherRestrictionAndOffset *)decodePublisherRestrictionBitChainStringFromBinary:(unsigned char*)binaryCharBuffer fromIndex:(int)startIndex {
    ...
    NSInteger numPubRestriction = [CR_SPTIabTCFUtils BinaryToDecimal:binaryCharBuffer fromIndex:startIndex length:NUM_PUBLISHER_RESTRICTIONS_BIT_LENGTH];
    ...
    for (int i=0; i<numPubRestriction; i++) {
        NSInteger purposeId = [CR_SPTIabTCFUtils BinaryToDecimal:binaryCharBuffer fromIndex:totalOffset length:PUBLISHER_RESTRICTIONS_PURPOSE_ID_BIT_LENGTH];
        totalOffset += PUBLISHER_RESTRICTIONS_PURPOSE_ID_BIT_LENGTH;
        
        
        PublisherRestrictionType restrictType = [CR_SPTIabTCFUtils BinaryToDecimal:binaryCharBuffer fromIndex:totalOffset length:PUBLISHER_RESTRICTION_TYPE_BIT_LENGTH];
...
+ (SPTVendorIdsAndOffset *)decodeBitChainStringForPubRest:(unsigned char*)binaryCharBuffer fromIndex:(int)startIndex {
    ...
    NSInteger numEntries = [CR_SPTIabTCFUtils BinaryToDecimal:binaryCharBuffer fromIndex:totalOffset length:NUM_ENTRIES_BIT_LENGTH];
    ...
    for (int i = 0 ; i < (int)numEntries ; i++) {
        ...
        if (!isRangeAndNotSingleEntry) {
            NSInteger singleVendorId = [CR_SPTIabTCFUtils BinaryToDecimal:binaryCharBuffer fromIndex:totalOffset  length:START_OR_ONLY_VENDOR_ID_BIT_LENGTH];
            totalOffset += START_OR_ONLY_VENDOR_ID_BIT_LENGTH;
            [vendorConsentIds addObject:[NSNumber numberWithInteger:singleVendorId]];
            
        } else {
            NSInteger startVendorId = [CR_SPTIabTCFUtils BinaryToDecimal:binaryCharBuffer fromIndex:totalOffset length:START_OR_ONLY_VENDOR_ID_BIT_LENGTH];
            totalOffset += START_OR_ONLY_VENDOR_ID_BIT_LENGTH;
            
            NSInteger endVendorId = [CR_SPTIabTCFUtils BinaryToDecimal:binaryCharBuffer fromIndex:totalOffset length:END_VENDOR_ID_BIT_LENGTH];
