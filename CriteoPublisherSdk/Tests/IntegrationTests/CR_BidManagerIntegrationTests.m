...
- (void)setUp {
  ...
  // TODO get rid of this once AdUnitHelper is injected DPP-3734
...
- (void)test_given3AdUnits_whenPrefetchBid_thenGet3Bids {
  ...
  XCTAssertNotNil([self.bidManager getBidThenFetch:self.cacheAdUnit1 withContext:self.contextData responseHandler:nil]);
  XCTAssertNotNil([self.bidManager getBidThenFetch:self.cacheAdUnit2 withContext:self.contextData responseHandler:nil]);
  XCTAssertNotNil([self.bidManager getBidThenFetch:self.cacheAdUnit3 withContext:self.contextData responseHandler:nil]);
}
...
- (void)test_givenPrefetchingBid_whenGetBid_thenDontFetchBidAgain {
  ...
  [self.bidManager getBidThenFetch:self.cacheAdUnit1 withContext:self.contextData responseHandler:nil];
  [self.dependencyProvider.threadManager waiter_waitIdle];
...
- (void)test_givenPrefetchingBid_whenGetImmediateBid_shouldProvideBidWithTtlSetToDefaultOne {
  ...
  CR_CdbBid *cachedBid = [self.bidManager getBidThenFetch:self.cacheAdUnit1
                                              withContext:self.contextData responseHandler:nil];
