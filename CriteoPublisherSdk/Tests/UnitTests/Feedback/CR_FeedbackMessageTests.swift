...
class CR_FeedbackMessageTests: XCTestCase {
  ...
  func testEmptyObjectsEqualityAndHash() {
    assertTwoMessagesObjectAndHashEquality(
      CR_FeedbackMessage(),
      CR_FeedbackMessage())
  }
  ...
  private func assertMessageEqualityAfterEncodingAndDecoding(
    message: CR_FeedbackMessage, file: StaticString = #file, line: UInt = #line
  ) {
  ...
  private func archiveAndUnarchive(message: CR_FeedbackMessage) throws -> CR_FeedbackMessage? {
    let data = try NSKeyedArchiver.archivedData(
      withRootObject: message, requiringSecureCoding: true)
