...
+ (CR_NativeAssets *)loadNativeAssets:(NSString *)fileName {
  NSError *e = nil;
  NSURL *jsonURL = [[NSBundle bundleForClass:[self class]] URLForResource:fileName
                                                            withExtension:@"json"];
...
- (BOOL)testHashAndIsEqualForUnequalObjects:(NSDictionary *)dict key:(id)key modValue:(id)modValue {
  ...
  NSLog(@"assets.hash = %lu, modAssets.hash= %lu", (unsigned long)assets.hash,
        (unsigned long)modAssets.hash);
...
- (void)checkIsAllNormal:(CR_NativeAssets *)assets {
  ...
  XCTAssertEqualObjects(assets.products[0].clickUrl,
  ...
  XCTAssertEqualObjects(assets.products[1].clickUrl,
  ...
  XCTAssertEqualObjects(assets.advertiser.logoClickUrl,
                        @"https://cat.sv.us.criteo.com/delivery/ckn.php?");
...
- (void)testWrongTypes {
  NSDictionary *badJsonDict1 =
      @{@"products" : @(1), @"advertiser" : @(1), @"privacy" : @(1), @"impressionPixels" : @(1)};
...
- (void)testUnequalObjects {
  NSDictionary *modProductDict2 = [self.productDict2 cr_dictionaryWithNewValue:@"$99999"
                                                                        forKey:@"price"];
  ...
  NSDictionary *modAdvertiserDict = [self.advertiserDict cr_dictionaryWithNewValue:@"Blah blah"
                                                                            forKey:@"domain"];
  [self testHashAndIsEqualForUnequalObjects:self.jdict
                                        key:@"advertiser"
                                   modValue:modAdvertiserDict];

  NSDictionary *modPrivacyDict = [self.privacyDict cr_dictionaryWithNewValue:@"Oink!"
                                                                      forKey:@"longLegalText"];
  [self testHashAndIsEqualForUnequalObjects:self.jdict key:@"privacy" modValue:modPrivacyDict];

  NSDictionary *modImpressionPixelDict1 =
      [self.impressionPixelDict1 cr_dictionaryWithNewValue:@"Hannibal" forKey:@"url"];
  NSArray *modImpressionPixelArray = @[ modImpressionPixelDict1, self.impressionPixelDict2 ];
  [self testHashAndIsEqualForUnequalObjects:self.jdict
                                        key:@"impressionPixels"
                                   modValue:modImpressionPixelArray];
}
