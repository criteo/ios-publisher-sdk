...
- (void)testGoogleMediationParametersNormal {
    ...
    CRGoogleMediationParameters *gmp = [CRGoogleMediationParameters
           parametersFromJSONString:@"{\"cpId\":\"B-056946\", \"adUnitId\": \"/140800857/Endeavour_320x50\"}"
           error:&error];
    XCTAssertNotNil(gmp);
...
- (void)testGoogleMediationParametersNilCpid {
    NSError *error = [NSError new];
    CRGoogleMediationParameters *gmp = [CRGoogleMediationParameters
            parametersFromJSONString:@"{\"cpidx\":\"B-056946\", \"adUnitId\": \"/140800857/Endeavour_320x50\"}"
            error:&error];
    XCTAssertNil(gmp);
    XCTAssertEqual(error.code, kGADErrorInvalidArgument);
}
...
- (void)testGoogleMediationParametersNilAdUnitId {
    NSError *error = [NSError new];
    CRGoogleMediationParameters *gmp = [CRGoogleMediationParameters
           parametersFromJSONString:@"{\"cpId\":\"B-056946\", \"adUnitIDx\": \"/140800857/Endeavour_320x50\"}"
           error:&error];
    XCTAssertNil(gmp);
    XCTAssertEqual(error.code, kGADErrorInvalidArgument);
}
...
- (void)testGoogleMediationParametersTotallyBadJson {
    NSError *error = [NSError new];
    CRGoogleMediationParameters *gmp = [CRGoogleMediationParameters
           parametersFromJSONString:@"ASFQ$RT @#VSDVC "
           error:&error];
    XCTAssertNil(gmp);
    XCTAssertEqual(error.code, kGADErrorInvalidArgument);
}
