...
- (instancetype)initWithResponse:(NSURLResponse *)response data:(NSData *)data error:(NSError *)error {
    if (self = [super init]) {
        ...
        _httpResponse = [_response isKindOfClass:NSHTTPURLResponse.class] ? (NSHTTPURLResponse *) _response : nil;
        _data = [data copy];
        _error = [error copy];
    }
    return self;
...
- (NSString *)title {
    ...
    return [NSString stringWithFormat:@"⬇️ RESP: %ld %@", (long) self.httpResponse.statusCode, host];
}
...
- (NSString *)detail {
    ...
    if (self.httpResponse) {
        detail = [detail stringByAppendingFormat:@"Status: %ld %@\n\n",
                                                 (long) self.httpResponse.statusCode,
                                                 [NSHTTPURLResponse localizedStringForStatusCode:self.httpResponse.statusCode]];
    }

    // Body
    NSString *body = self.data ? [[NSString alloc] initWithData:self.data encoding:NSUTF8StringEncoding] : nil;
